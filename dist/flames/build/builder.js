...
function chain(actions) {
    function transform(file, encoding, callback) {
        ...
        if (file.flContents) {
            let relativeName = (path.relative(file.base, file.path) || '').trim();
            let dirName = path.dirname(relativeName);
            let name = path.basename(relativeName, '.json');
            // Set file props
            if (!('parents' in file)) {
                let parents = dirName.split('/') || [];
                if (parents[0] == '.')
                    parents.splice(0);
